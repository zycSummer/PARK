...
layui.use(['request', 'form', 'element', 'layer', 'jquery', 'laydate'], function () {
  ...
  function init() {
    ...
    dm.enableAnimation();//启动全局动画定时器，参数interval指定全局动画间隔
    ...
    setTimeout(function () {
      var animationObj = dm.toDatas(function (data) {
      ...
      var opnameData = dm.toDatas(function (data) {
        //过滤函数构建新的元素集合并返回
  ...
  function connect(opnameData, opArr, dataModel) {
    var sock = new SockJS('/jet-stomp-websocket'); // 获取 STOMP 子协议的客户端对象

    stompClient = Stomp.over(sock); // 向服务器发起websocket连接并发送CONNECT帧
    ...
    stompClient.connect({}, function (frame) {
      ...
      stompClient.send("/htweb/queryActualMonitorData", {}, JSON.stringify(data));
    ...
    stompClient.onclose = function (ev) {
      console.log("closed"); // 已经关闭的情况，不重连
  ...
  function getHisData(time1, time2) {
    ...
    var start = moment(data.start, 'YYYY-MM-DD HH:mm:ss').valueOf();
    var end = moment(data.end, 'YYYY-MM-DD HH:mm:ss').valueOf();
