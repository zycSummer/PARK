 ...
 * @Last Modified time: 2019-12-17 16:30:08
 */
layui.use(['request', 'form', 'element', 'layer', 'jquery', 'tree', 'laydate'], function () {
  ...
  var energyTypeId = '',
      energyLoadParaId; //能源种类

  var treeAjaxed = false,
      selectAjaxed = false;
    ...
    if (siteObj) {
      ...
      request.service({
        ...
        $.each(res.data, function (i0, v0) {
          if (i0 == 0) {
            energyLoadParaId = v0['energyLoadParaId'];
            energyTypeId = v0['energyTypeId'];
            str += '<li data-id="' + v0['energyTypeId'] + '" data-paraId="' + v0['energyLoadParaId'] + '" class="h_item h_active">';
          } else {
            str += '<li data-id="' + v0['energyTypeId'] + '" data-paraId="' + v0['energyLoadParaId'] + '" class="h_item">';
          }
        ...
        element.on('collapse(tree)', function (data) {
          if (data.show == false) {
            ...
            if (tree1) {
              ...
              if (nodes.length > 0) {
                tree1.checkNode(nodes[0], true, false);
              }
            }
          }
        });
        treeAjaxed = true;
        if (selectAjaxed) getChartData();
      })["catch"](function (err) {
      disLoad();
      console.log(err);
    });
  ...
  function getSelectParams(energyLoadParaId) {
    if (siteObj && energyTypeId) {
      selectAjaxed = false;
      load();
      request.service({
        ...
        paramsSelect.setValue([energyLoadParaId]);
        selectAjaxed = true;
        if (treeAjaxed) getChartData();
      })["catch"](function (err) {
      disLoad();
      console.log(err);
    });
  ...
  function init1() {
    ...
    var yNameArr = []; //一个参数一张图（折线图）进行展示。如果勾选的多个节点有同一参数，则这张图上展示多条曲线。
        ...
        formatter: function formatter(params) {
          ...
          for (var i = 0, l = params.length; i < l; i++) {
            if (params[i].value != null) {
              relVal += "<span  style='display:inline-block;margin-right:5px;" + "border-radius:10px;width:10px;height:10px;background-color:" + params[i].color + ";'>" + '<span  style="display:block;padding-left:15px;' + 'margin-top:-4px">' + params[i].seriesName + '[' + yNameArr[params[i].axisIndex].name + ']' + ' : ' + params[i].value + yNameArr[params[i].axisIndex].unit + '</span>' + "</span>" + '<br>';
            }
          }
