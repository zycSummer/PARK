 ...
 */
layui.use(
    ["form", "element", "layer", "jquery", "table", "laypage", "laydate"],
    function () {
        var form = layui.form,
            layer = layui.layer,
            $ = layui.jquery,
        ...
        function renderHeadTitle() {
            ...
            if (sessionStorage.getItem("parkId")) {
                ...
                switch (objSiteType) {
                    case "PARK":
                        $(".content_main_title").text(
                            JSON.parse(sessionStorage.getItem("parkId")).name + " - 用量信息"
                        );
                        break;

                    default:
                        $(".content_main_title").text(
                            JSON.parse(sessionStorage.getItem("parkId")).title + " - 用量信息"
                        );
                        break;
                }
            }
        ...
        function getleftChartData() { //获取饼图数据
            var formData = {};
        ...
        function renderLeftChart(list, nameList) {
            //渲染左侧饼图
            var leftChart = echarts.init(
                document.getElementById("content_left_chart")
            );
            var option = {
                tooltip: {
                    trigger: "item",
                    ...
                    if (typeData.length > 0) {
                        typeData.forEach(function (tpye, index) {
                            switch (index) {
                                case 0:
                                    tpyeHtml +=
                                        '<a  class="typeObj layui-this"   data-objid="' +
                                        tpye.energyTypeId +
                                        '">' +
                                        tpye.energyTypeName +
                                        "</q>";
        ...
        function getRightChartData() { //获取右侧图表数据
            var formData = {};
            formData.objType = JSON.parse(sessionStorage.getItem("parkId")).type;
            formData.objId = JSON.parse(sessionStorage.getItem("parkId")).id;
