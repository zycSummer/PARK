 ...
 * @Last Modified time: 2019-12-03 14:34:10
 */
...
layui.use(['form', 'element', 'layer', 'table', 'jquery', 'laypage', 'laydate'], function () {
    ...
    function getAlarmData() { //ajax获取 数据
    ...
    $("#edit_btn_conf").on('click', function () {
        ...
        request.service({
            ...
            .then(function (res) {
                ...
                $("#addIsUSe").val(String(updateData.isUse)); //是否启用
                $("#addMultiConditionsLogic").val(updateData.multiConditionsLogic); //多条件间关系
    ...
    //模态框事件
    function showConfDialog(editId, title) { //模态框调用事件
        layer.open({
            type: 1,
            title: title,
            closeBtn: 1,
            ...
            yes: function (index) {
                ...
                var isUse = $("#addIsUSe").val(); //是否启用
                var multiConditionsLogic = $("#addMultiConditionsLogic").val(); //多条件间关系
                ...
                if(msgRecv){ //当存在短信接收人时
                  ...
                  addMsgRecvArr =msgRecv.split(';');
                     if(addMsgRecvArr.length>0){
                       for (let i = 0; i < addMsgRecvArr.length; i++) {
    ...
    $("#del_btn_conf").on('click', function () {
        ...
        layer.open({
            ...
            content: '<div id="park_select_tree"> <form class="layui-form" style="margin:30px;">' +
                '   <div class="layui-form-item "><label>报警标识：</label>' +
                '<div class="layui-inline">' + checkStatusData[0].alarmId + '</div></div>' +
                '   <div class="layui-form-item "><label>报警名称：</label>' +
                '<div class="layui-inline" >' + checkStatusData[0].alarmName + '</div></div>' +
                '   <div class="layui-form-item " style="color:red;">确定要删除此报警吗？此报警下配置的报警条件也将一并删除。</div>' +
    ...
    $("#search_btn_data").on('click', function () { //查询点击事件
    ...
    function getDataSourceList() { //根据所配置的报警信息 获取数据源信息
        var tnum = parseInt(($(".tab_content").height() - 60) / 42); //动态生成表格展示条数
        var formData = {};
        formData.limit = pageLimit ? pageLimit : tnum;
        formData.page = pageNum;
        ...
        request.service({
            ...
            .then(function (res) {
                ...
                renderDataSourceTable(rightTabData, formData.limit);
    ...
    function renderDataSourceTable(data, limit) { //渲染数据源表格
        table.render({
    ...
    function renderDataSourcePage(count, tnum) { //渲染数据源分页
        laypage.render({
    ...
    function getCompanyList() {
        ...
        load();
        if (selectType == 'PARK') { //当为园区时请求获取当前园区下的企业
            request.service({
                    method: 'get',
    ...
    //meterRtdbId
    function renderCompanyList(list, type) { //渲染企业列表
    ...
    $("#add_btn_meter").on('click', function () { //仪表查询事件
        getMeterData(meter_objType, meter_objId);

    })

    function getMeterData(objType, objId) { //根据所选企业获取仪表数据
        var formData = {};
    ...
    function renderMeterTab(list) { //根据所获取的数据 渲染仪表表格
        var meterHtml = '';
        if (list.length > 0) {
    ...
    $("#add_btn_parameter").on('click', function () { //参数查询点击事件
        getParameterTabData(parameter_energyTypeId);
    })

    function getParameterTabData(energyTypeId) { //根据所选择仪表获取参数数据
         var formData ={}; 
    ...
    function renderParameterTab(list) { //根据所获取的数据渲染参数列表
        var parameterHtml = '';
        if (list.length > 0) {
            list.forEach(function (item) {
    ...
    element.on('tab(modelSelect)', function (data) { //tab 选项卡点击事件
        if (data.index == 2) { //当选项卡为参数时  显示按钮
            $(".parameter_btns").show();
        } else {
            $(".parameter_btns").hide();
        }
    ...
    $(document).on("click",'.datasoucre_tr',function(){  //仪表参数选中事件
        $('.datasoucre_tr').removeClass('datasoucre_tr_status');
         $(this).addClass('datasoucre_tr_status');
     })
 
 
     $(".parameter_btn_type").on('click', function () {  //添加至所选之前
         if ($(".parameter_btn_status").length > 0) { //当存在所选参数时 判断 仪表参数列表是否存在
            ...
            if( document.getElementsByClassName('datasoucre_tr').length>0){ //当仪表参数长度大于1时
     ...
     var timer = null;
     $(document).on("click",'.parameter_tr',function(){  //参数选中事件
         var that =this;
         clearTimeout(timer);
     ...
     $(document).on("dblclick", '.parameter_tr', function () { //参数双击事件
         clearTimeout(timer); 
    ...
    $("#formula_check").on('click', function () { //公式检查事件
        ...
        if (formulaVal.length > 0) {
            for (var index = 0; index < formulaVal.length; index++) {
                let element = formulaVal[index];
                if (element == '?') {
                    selectValArr.push(element);
                }
            }

            if (dataSocureLength == 1) { //如果仪表参数列表中只有一个仪表参数，公式可以为空，但不能只有1个问号
                if (selectValArr.length == 1) {
                    return layer.msg('公式中的问号个数不能为一个')
                }
            }
            if (selectValArr.length != dataSocureLength) { //检查数据源-仪表参数列表的个数 和 公式中的问号个数是否一致，不一致时提醒用户
    ...
    $("#add_btn_data").on('click', function () { //新增点击事件
    ...
    $("#edit_btn_data").on('click', function () { //修改报警条件点击事件
    ...
    function showDataSourceDialog(editId, title) { //数据源模态框调用事件
        layer.open({
            type: 1,
            title: title,
            closeBtn: 1,
    ...
    $("#del_btn_data").on('click', function () {
        ...
        layer.open({
            ...
            area: ['500px', '600px'], //宽高
            content: '<div id="park_select_tree"> <form class="layui-form" style="margin:30px;">' +
                '   <div class="layui-form-item "><label>报警条件标识：</label>' +
                '<div class="layui-inline">' + checkStatusData[0].alarmConditionId + '</div></div>' +
                '   <div class="layui-form-item "><label>数据源：</label>' +
                '<div class="layui-inline" >' + checkStatusData[0].dataSource + '</div></div>' +
                '   <div class="layui-form-item "><label>数值类型：</label>' +
                '<div class="layui-inline" >' + valueTypeText + '</div></div>' +
                '   <div class="layui-form-item "><label>条件1操作符：</label>' +
                '<div class="layui-inline" >' + checkStatusData[0].condition1Op + '</div></div>' +
                '   <div class="layui-form-item "><label>条件1值：</label>' +
                '<div class="layui-inline" >' + checkStatusData[0].condition1Value + '</div></div>' +
                '   <div class="layui-form-item "><label>条件间关系：</label>' +
                '<div class="layui-inline" >' + checkStatusData[0].conditionsLogic + '</div></div>' +
                '   <div class="layui-form-item "><label>条件2操作符：</label>' +
                '<div class="layui-inline" >' + checkStatusData[0].condition2Op + '</div></div>' +
                '   <div class="layui-form-item "><label>条件2值：</label>' +
                '<div class="layui-inline" >' + checkStatusData[0].condition2Value + '</div></div>' +
                '   <div class="layui-form-item "><label>报警信息：</label>' +
                '<div class="layui-inline" >' + checkStatusData[0].alarmMsg + '</div></div>' +
                '   <div class="layui-form-item "><label>备注：</label>' +
                '<div class="layui-inline" >' + checkStatusData[0].memo + '</div></div>' +
                '   <div class="layui-form-item " style="color:red;">确定要删除此报警条件吗？</div>' +
