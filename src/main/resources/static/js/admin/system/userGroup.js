 ...
 * @Last Modified time: 2019-11-26 15:34:45
 */
layui.config({
    ...
    function getUserGroupData() { //ajax获取 数据
    ...
    table.on('radio(role_table)', function(obj){  //表格单选切换事件 
        getRightTree()

      });
      var  rightTree;
     function  getRightTree(){  //获取右侧用户组权限树
        ...
        .then(function (res) {
         ...
         var   roleTreeData=[];
               ...
               rightTree ='';
               roleTreeData= handleRoleTreeData(roleTreeData).data;
             ...
             //重新加载树
              var checkedData  =handleRoleTreeData(roleTreeData).checkedData;
     ...
     var addRightTree;
      function getAllSite(){
        ...
        .then(function (res) {
            ...
            addRoleTreeData= handleRoleTreeData(addRoleTreeData).data;
    ...
    $('#del_btn').click(function () { //删除点击事件
        ...
        layer.open({
            ...
            content: '<div id="park_select_tree"> <form class="layui-form" style="margin:30px;">' +
                '   <div class="layui-form-item "><label>用户组ID：</label>' +
                '<div class="layui-inline">' + tableSelect[0].userGroupId + '</div></div>' +
                '   <div class="layui-form-item "><label>用户组名称：</label>' +
                '<div class="layui-inline" >' + tableSelect[0].userGroupName + '</div></div>' +
                '   <div class="layui-form-item " style="color:red;">确定要删除此报警吗？此用户组与对象(园区、企业)的关联关系一并删除! 。用户管理中涉及到的用户组关联需要重新手动设置! </div>' +
    ...
    function handleRoleTreeData(data){ //处理权限数据
       ...
       forfun(data);
        return {"data":data,"checkedData":checkedData};
    }

    //提示弹框点击 -用户组ID
    $("#add_userGroupId_tip").on("click", function () {
        layer.tips('字母、数字组合，长度不超过20。', '#add_userGroupId_tip');
    })
    //提示弹框点击 -用户组名称
