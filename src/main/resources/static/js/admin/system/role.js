 ...
 * @Last Modified time: 2019-12-30 15:48:02
 */
layui.config({
  ...
  $("#add_btn").on('click', function () {
    //新增点击事件
    $("#add_role_code").attr('disabled', false);
    $("#add_role_code").val('');
    $("#add_role_name").val('');
    $("#memo").val('');
  ...
  $("#edit_btn").on('click', function () {
    //修改点击事件
    $("#add_role_code").attr('disabled', true);
  ...
  $('#del_btn').click(function () {
    ...
    layer.open({
      ...
      area: ['320px', '300px'],
      //宽高
      content: '<div id="park_select_tree"> <form class="layui-form" style="margin:30px;">' + '   <div class="layui-form-item "><label>角色标识：</label>' + '<div class="layui-inline">' + tableSelect[0].roleId + '</div></div>' + '   <div class="layui-form-item "><label>角色名称：</label>' + '<div class="layui-inline" >' + tableSelect[0].roleName + '</div></div>' + '   <div class="layui-form-item " ><span style="color:red;">确定要删除此角色吗？此角色与菜单、功能的关联关系一并删除! 用户管理中涉及到的角色关联需要重新手动设置! </span> </div>' + '</form></div>',
      success: function success() {},
      yes: function yes(index) {
        load();
        request.service({
  ...
  $("#authority_maintenance_btn").on('click', function () {
    //权限维护事件
    var tableSelect = table.checkStatus('role_table').data;
  ...
  function showModelAuthBox(roleId, title) {
    //模态框调用事件
    layer.open({
      type: 1,
      title: title,
      ...
      btn: ['保存', '关闭'],
      success: function success() {
        $('#role_auth').removeClass('layui-hide').addClass('layui-show'); //根据角色code 获取权限集
    ...
    if (obj.event === "menuClick") {
      ...
      switch (selectStatus) {
        ...
        case false:
          ...
          if (menuObjData.btnList.length > 0) {
            //当子类为菜单时  下一级 为按钮时
  ...
  function handleRoleTreeData(data) {
    //处理权限数据
    var checkedData = [];
    ...
    if (data.length > 0) {
      var forfun = function forfun(arr) {
        arr.forEach(function (item) {
          ...
          if (item.children) {
            if (item.children[0].type == 'function') {
              //当子类为按钮时
  ...
  $("#add_role_code_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过20。', '#add_role_code_tip');
  }); //弹框 提示点击 

  $("#add_role_name_tip").on("click", function () {
    layer.tips('长度不超过30。', '#add_role_name_tip');
  });
  initFun(); //页面加载时执行初始函数 
}); // https://www.jianshu.com/p/e45d72ce2a78