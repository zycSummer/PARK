 ...
 * @Last Modified time: 2019-11-22 15:21:42
 */
 ...
 var  tableId ='treeTable';
   ...
   function disLoad(){
    ...
    function initFun() {
     ...
     function getEnergyTypeData(){  //ajax获取 数据  
          load();
          var formData ={};
     ...
     $("#add_btn_type").on("click",function(){  //能源种类新增点击事件
     ...
     $("#edit_btn_type").on("click",function(){//修改能源种类
    ...
    $('#del_btn_type').click(function () { //能源种类删除点击事件
        ...
        layer.open({
            ...
            content: '<div id="park_select_tree"> <form class="layui-form" style="margin:30px;">' +
                '   <div class="layui-form-item "><label>能源种类标识：</label>' +
                '<div class="layui-inline">' + tableSelect[0].energyTypeId + '</div></div>' +
                '   <div class="layui-form-item "><label>能源种类名称：</label>' +
                '<div class="layui-inline" >' + tableSelect[0].energyTypeName + '</div></div>' +
                '   <div class="layui-form-item " style="color:red;">确定要删除此能源种类吗？此能源种类下的参数将一并删除！</div>' +
                '</form></div>',
            success: function () {
            },
            yes: function (index) {
    ...
    table.on('radio(energyTypeTable)', function(obj){  //表格单选切换事件 
    ...
    function getParaData(energyTypeId){ //获取参数列表
        var formData ={};
        formData.energyTypeId =energyTypeId;
    ...
    $("#add_btn_para").on('click',function(){//能源种类参数添加事件
        var tableSelect = table.checkStatus('energyTypeTable').data;
        if (tableSelect.length == 0) {
            return layer.msg('请选择能源种类！')
        }
    ...
    $("#edit_btn_para").on('click',function(){  //按钮修改事件
    ...
    function showParaModel(editId,title) {//模态框调用事件
        layer.open({
            type: 1,
            title: title,
            closeBtn: 1,
    ...
    $('#del_btn_para').click(function () { //能源参数删除事件
        var tableSelectPara = table.checkStatus('paraTable').data;
        var tableSelect = table.checkStatus('energyTypeTable').data;
        if (tableSelectPara.length == 0) {
        ...
        layer.open({
            ...
            content: '<div id="park_select_tree"> <form class="layui-form" style="margin:30px;">' +
                ...
                '<div class="layui-inline">['+tableSelect[0].energyTypeId+']'+tableSelect[0].energyTypeName+'</div></div>' +
                '   <div class="layui-form-item "><label>能源参数标识：</label>' +
                '<div class="layui-inline">' + tableSelectPara[0].energyParaId + '</div></div>' +
                '   <div class="layui-form-item "><label>能源参数名称：</label>' +
                '<div class="layui-inline" >' + tableSelectPara[0].energyParaName + '</div></div>' +
                '   <div class="layui-form-item " style="color:red;">确定要删除此能源种参数吗？</div>' +
                '</form></div>',
            success: function () {
            },
            yes: function (index) {
     ...
     $("#add_energyTypeId_tip").on("click", function () {
        layer.tips('字母、数字组合，长度不超过20。', '#add_energyTypeId_tip');
    })
    //提示弹框点击 
    ...
    $("#addEnergyUsageParaIdTip").on("click", function () {
        layer.tips('字母、数字组合，长度不超过20。', '#addCo2CoeffTip');
    })
    $("#addEnergyLoadParaIdTip").on("click", function () {
        layer.tips('字母、数字组合，长度不超过20。', '#addEnergyLoadParaIdTip');
    })
    $("#addSortIdTip").on("click", function () {
        layer.tips('字母、数字组合，长度不超过20。 系统按照字符串升序', '#addSortIdTip');
    })
    $("#paraEnergyParaIdTip").on("click", function () {
        layer.tips('字母、数字组合，长度不超过20。', '#paraEnergyParaIdTip');
    })
