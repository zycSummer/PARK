 ...
 * @Last Modified time: 2019-12-24 14:36:23
 */
layui.use(['request', 'form', 'element', 'layer', 'jquery', 'table', 'tree'], function () {
  ...
  $('.param_delete').on('click', function () {
    ...
    $('#param_delete_name').html(tableSelect[0].displayName); // 数值类型
  ...
  $("#report_id_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过20,在园区/当前企业内唯一。', '#report_id_tip');
  }); //报表弹框 提示点击 - 报表名称
  ...
  $("#report_sortId_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过10，系统按照字符串升序', '#report_sortId_tip');
  }); //展示参数弹框 提示点击 - 展示名称
  ...
  $("#param_sortId_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过10，系统按照字符串升序', '#param_sortId_tip');
  }); //展示对象弹框 提示点击 - 节点ID

  $("#obj_nodeId_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过20,在当前报表内唯一。', '#obj_nodeId_tip');
  }); //展示对象弹框 提示点击 - 节点名称
  ...
  $("#obj_sortId_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过10，系统按照字符串升序', '#obj_sortId_tip');
  }); //监听提交
    ...
    layer.open({
      ...
      success: function success() {
        ...
        if (type == 'edit') {
          ...
          $("#param_name").val(tableSelect[0].displayName); // 数值类型
      ...
      yes: function yes(index) {
        var tableSelect = table.checkStatus('param_table').data;
        var val = tree1.getSelectedNodes(); // 数值类型
        ...
        if (res.data) {
          ...
          table.render({
            ...
            cols: [[//表头
            {
              ...
              templet: function templet(d) {
                if (d.timeValue == 'Y') {
                  return '<input type="checkbox" checked lay-skin="primary" disabled>';
                } else {
