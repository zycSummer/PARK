 ...
 * @Last Modified time: 2019-12-17 16:27:30
 */
layui.use(['request', 'element', 'layer', 'request', 'laydate'], function () {
  ...
  var ToggleOverview = function ToggleOverview(graphView) {
    var self = this;
    ...
    self.setContentBackground("white"); // 通过getMaskBackground和setMaskBackground获取和设置可见区域遮罩背景颜色
  ...
  function init() {
    ...
    overview.getView().style.top = ''; // window.addEventListener('resize', function(e) {
    //     gv.invalidate();
    // }, false);
  ...
  function createNode(data, parent, child) {
    //创建Node节点
  ...
  function createEdge(source, target, data, color, group) {
    ...
    edge.s({
      ...
      'edge.color': '#2C3E50',
      'edge.type': 'h.v',
      //字符串类型的连线类型 boundary ripple h.v v.h ortho flex extend.east
      'edge.corner.radius': 10,
      //控制拐角曲线弧度
      // 'edge.ripple.size':1,
      // 'edge.ripple.length':80,
      'edge.center': true,
      //默认值为false代表从矩形边缘开始走向，true则起始和结束于中心
      'edge.flow': true,
      'edge.flow.step': 1,
      // 控制流动的步进，默认为3
      'edge.flow.reverse': true,
      //值为true或flase，控制反向流动，默认值为false
      'edge.flow.autoreverse': true,
      //值为true或flase，正向流动和反向流动是否交替进行，默认值为false
      'edge.flow.count': 1,
      // 控制流动组的个数，默认为1
      'edge.flow.element.count': 10,
      //每个流动组中的元素的个数，默认为10
      // 'edge.flow.element.background':'red', //流动组中元素的背景颜色，默认为rgba(255, 255, 114, 0.4)
      'edge.flow.element.max': 7,
      //流动组中最大元素的尺寸，默认为7
      'edge.flow.element.min': 2,
      //流动组中最小元素的尺寸，默认为0
      'edge.flow.element.space': 4,
      //流动组中元素的间隔，默认为3.5
      'edge.flow.element.autorotate': true,
      //值为true或false，控制流动组中的元素是否与路径的方向保持一致，默认为false
  ...
  function initTime() {
    ...
    $("#timeDiv").html('<input type="text" class="layui-input dateInput" id="timeInput" value="' + time1 + '">');
    laydate.render({
      elem: '#timeInput',
      type: type,
      lang: 'cn',
