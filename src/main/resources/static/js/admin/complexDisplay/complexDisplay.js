...
layui.use(['request', 'form', 'element', 'layer', 'jquery', 'laydate'], function () {
  ...
  window.openHistory = function (event, data, view) {
    ...
    dialog.setConfig({
      ...
      content: document.getElementById("historyChart"),
      //指定对话框的内容，可以是html文本，或DOM对象，或HT组件对象（自动调用其invalidate）
      ...
      closable: true,
      //可选值为true/false，表示是否显示关闭按钮
      maximizable: true,
      //可选值为true/false，表示对话框是否可被最大化
      resizeMode: "wh",
      ...
      buttons: [{
        ...
        action: function action(button, e) {
          dialog.hide(); // dialog.validate();
        }
      }],
      buttonsAlign: "center",
      //指定对话框按钮的对齐方式，可选值为left/right/center
      action: function action(item, e) {//全局回调函数，当点击按钮时，单个按钮的回调函数执行完成后再执行此全局回调函数，参数为函数，格式为：function(button, event) { }，button为点击的按钮，event为原生的事件对象(一般为MouseEvent)
        // console.log(item, e);
      }
    });
    dialog.show();
    ...
    getHisData(time1, time2); //增加事件监听器，参数为监听函数，派发的事件种类可以查看简单示例中的Demo
    ...
    if (siteObj) {
      ...
      request.service({
        ...
        if (res.one) {
          ...
          var json = res.one.cfgPic ? res.one.cfgPic : {
            "v": "7.0.1",
            "p": {
          ...
          var stretch = dataModel2.toDatas(function (data) {
            return data.a('stretch');
          });
          ...
          var opnameData2 = dataModel2.toDatas(function (data0) {
            //过滤函数构建新的元素集合并返回
          ...
          dialog.show(); //增加事件监听器，参数为监听函数，派发的事件种类可以查看简单示例中的Demo
          ...
          dialog.addEventListener(function (e) {
            ...
            if (e.kind == 'maximize' || e.kind == 'restore' || e.kind == 'betweenResize') {} // changeData();
            // console.log(e);
  ...
  window.changeParas = function (event, data) {
    var opnameData = dataModel.toDatas(function (data) {
      //过滤函数构建新的元素集合并返回
      return data.a('paras');
    });
    paraDataList = [];
  ...
  function getData() {
    ...
    if (siteObj) {
      ...
      request.service({
        ...
        if (res.one) {
          htData = res.one.cfgPic ? res.one.cfgPic : {
            "v": "7.0.1",
            "p": {
              "autoAdjustIndex": true,
              "hierarchicalRendering": true
  ...
  function init(obj) {
    ...
    setTimeout(function () {
      var animationObj = dataModel.toDatas(function (data) {
        return data.a('animation');
      });
  ...
  function connect(opnameData, dm) {
    // 建立连接对象（还未发起连接）
    var sock = new SockJS('/jet-stomp-websocket', null, {
      'timeout': 60000
    }); // 获取 STOMP 子协议的客户端对象

    stompClient = Stomp.over(sock); // stompClient.heartbeat.outgoing = 10;  // client will send heartbeats every 20000ms
    ...
    opnameData.each(function (data6) {
      ...
      if (op && url && stompClient) {
        if (data6.a('interval') == 0) {
          ...
          } else if (url == '/htweb/loadTodayHistoryValue') {
            //实时负荷
            stompClient.send(url, {}, op);
        ...
        switch (url) {
          case '/htweb/energyTodayDiffValue':
            //中下角(各种能源或标煤 瞬时量 的当日差值)
            if (!sub1) {
          ...
          case '/htweb/energyRealTimeLoadRanking':
            //实时负荷排名
            if (!sub2) {
          ...
          case '/htweb/energyTodayUsageRanking':
            //当日能耗排名
            if (!sub5) {
          ...
          case '/htweb/loadTodayHistoryValue':
            // 当天实时负荷
            if (!sub4) {
          ...
          case '/htweb/energyRealTimeValue':
            // 综合展示最新值查询
            if (!sub4) {
