 ...
 * @Last Modified time: 2019-12-19 09:24:23
 * 公共组件 js
 */
layui.config({
  base: "/public/js/",
...
function handleNum(data, page, limit) {
  var numArr = [];

  if (data.length > 0) {
    for (var index = 0; index < data.length; index++) {
      var objNum = data[index];

      if (page == 1) {
        objNum.number = index + 1;
      } else {
        objNum.number = (page - 1) * limit + index + 1;
      }
...
function showCompanySelect() {
  //展示企业选择
  $(".comapnyDiv").removeClass("comapnyDivHide");
  $(".comapnyDivTitle").removeClass("comapnyDivHide");
}

function hideCompanySelect() {
  //隐藏企业选择
  $(".comapnyDiv").addClass("comapnyDivHide");
  $(".comapnyDivTitle").addClass("comapnyDivHide");
}