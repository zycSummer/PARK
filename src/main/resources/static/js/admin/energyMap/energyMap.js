 ...
 * @Last Modified time: 2019-12-09 16:23:53
 */
...
layui.use(['request', 'form', 'element', 'layer', 'jquery'], function () {
	...
	$(".fullScreen").on('click', function () {
		...
		window.parent.layer.open({
			...
			success: function (layero, index) {
				sessionStorage.setItem("isCar","y")
	...
	function initMap(longitude, latitude, scale) { //初始地图
	...
	function renderRightTab(mapCompanyList) {
		...
		if (mapCompanyList.length > 0) {
			mapCompanyList.forEach(function (company, index) {
				switch (index) {
					case 0:
						companyHtml += '<div class="layui-colla-item">' +
							'<h2 class="layui-colla-title companyTitle"   data-objname="' + company.siteName + '" data-objid="' + company.objId + '" >' + company.siteName + '</h2>' +
							'<div class="layui-colla-content layui-show  companyObj ' + company.objId + '"> <i class="layui-icon">&#xe715;</i> ' + company.addr + '</div> </div>'
						break;

					default:
						companyHtml += '<div class="layui-colla-item">' +
							'<h2 class="layui-colla-title companyTitle"   data-objname="' + company.siteName + '" data-objid="' + company.objId + '">' + company.siteName + '</h2>' +
							'<div class="layui-colla-content  companyObj ' + company.objId + '"><i class="layui-icon">&#xe715;</i>    ' + company.addr + '</div> </div>'
						break;
				}
			...
			request.service({
				...
				.then(function (res) {
					...
					var mgs = '<div class="mapShow"><div class="mapShowTitle">' + objName + '</div><div class="mapShowContent">' +
						'<div class="mapShowConent_left"> <div><p class="titleIcon"></p><p>实时功率（电）：</p><p>' + electric + '</p><p>kW</p></div>' +
						'<div> <p class="titleIcon"></p><p>瞬时流量（水）：</p><p>' + water + '</p><p>t/h</p></div>' +
						'<div><p class="titleIcon"></p><p>瞬时流量（蒸汽）：</p><p>' + steam + '</p><p>t/h</p></div>' +
						'<div><p class="titleIcon"></p><p>年能耗标煤总量：</p><p>' + stdCoal + '</p><p>tce</p></div>' +
						'<div class="comparedP"><p class="titleIcon"></p><p >与上年同期：</p><p><span>' + lastIcon + '</span>' + lastYearCompareRate + '</p><p>%</p></div>' +
						'<div> <p class="titleIcon"></p><p>单位GDP能耗：</p><p>' + gdp + '</p><p>tce/万元</p></div></div>' +
						'<div class="mapShowConent_right"><img src="data:image/png;base64,' + res.one.icon + '"></div></div>' +
						'</div>';
	...
	function stopCarousel() {
		clearInterval(mapCarousel); //取消轮播
