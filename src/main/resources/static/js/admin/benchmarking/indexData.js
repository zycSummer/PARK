...
layui.use(['request', 'form', 'element', 'layer', 'jquery', 'laydate', 'table', 'laypage'], function () {
  ...
  $('#leftAdd').on('click', function () {
    ...
    $("#addbenchmarkingObjAbbrName").val('');
    form.render();
  ...
  $('#leftEdit').on('click', function () {
    ...
    request.service({
      ...
      $("#treeSortId").val(editObj.sortId);
      $("#tree_memo").val(editObj.memo);
      $("#addbenchmarkingObjAbbrName").val(editObj.benchmarkingObjAbbrName);
      $("#addWordLatitude").val(editObj.worldLatitude); //世界地图 -中心经度

      $("#addWordLongitude").val(editObj.worldLongitude); //世界地图 中心经度
  ...
  function showLeftModel(editId, title) {
    layui.layer.open({
      ...
      yes: function yes(index) {
        ...
        var worldLatitude = $("#addWordLatitude").val(); //世界地图 -中心经度

        var worldLongitude = $("#addWordLongitude").val(); //世界地图 中心经度

        var benchmarkingObjAbbrName = $("#addbenchmarkingObjAbbrName").val();
        ...
        if (!benchmarkingObjAbbrName) {
          return layer.msg("请输入对标对象简称");
        }
        ...
        formData.benchmarkingObjAbbrName = benchmarkingObjAbbrName;
        formData.sortId = sortId;
        formData.memo = memo;
        formData.id = editId;
        formData.worldLatitude = worldLatitude;
        formData.worldLongitude = worldLongitude;
  ...
  $('#leftDelete').click(function () {
    ...
    layer.open({
      ...
      area: ['320px', '350px'],
      //宽高
      content: '<div id="park_select_tree"> <form class="layui-form" style="margin:30px;">' + '   <div class="layui-form-item "><label>对标对象标识：</label>' + '<div class="layui-inline">' + val[0].benchmarkingObjId + '</div></div>' + '   <div class="layui-form-item "><label>对标对象名称：</label>' + '<div class="layui-inline">' + val[0].benchmarkingObjName + '</div></div>' + '   <div class="layui-form-item "><label>对标对象类型：</label>' + '<div class="layui-inline">' + benchmarkingObjTypename + '</div></div>' + '   <div class="layui-form-item " style="color:red;">确定要删除此对标对象吗？此对标对象的指标数据将一并删除!</div>' + '</form></div>',
      success: function success() {},
      yes: function yes(index) {
        var formData = {};
  ...
  function getRightData(benchmarkingObjId) {
    //ajax获取 数据
  ...
  $('#rightDel').click(function () {
    ...
    layer.open({
      ...
      area: ['380px', '260px'],
      //宽高
      content: '<div id="park_select_tree"> <form class="layui-form" style="margin:30px;">' + '   <div class="layui-form-item "><label>对标对象：</label>' + '<div class="layui-inline">' + objTitle + '</div></div>' + '   <div class="layui-form-item "><label>年：</label>' + '<div class="layui-inline">' + tableSelect[0].year + '</div></div>' + '   <div class="layui-form-item " style="color:red;">确定要删除此对标对象指标数据吗？</div>' + '</form></div>',
      success: function success() {},
      yes: function yes(index) {
        load();
        request.service({
  ...
  $("#benchmarkingObjId_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过20 ,在园区/当前企业内唯一', '#benchmarkingObjId_tip');
  }); //提示弹框点击 
  ...
  $("#addbenchmarkingObjAbbrNameTip").on("click", function () {
    layer.tips('长度不超过15', '#addbenchmarkingObjAbbrNameTip');
  });
  $("#tree_sortId_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过20 系统按照字符串升序', '#tree_sortId_tip');
  });
