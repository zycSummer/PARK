 ...
 * @Last Modified time: 2019-12-19 10:46:03
 */
layui.use(['form', 'element', 'layer', 'table', 'jquery', 'laypage', 'laydate'], function () {
  ...
  $('#add_btn').on('click', function () {
    ...
    $("#manufacturer").val('');
    $("#model").val('');
    $("#location").val(''); // $("#productionDate").val('');
    // $("#firstUseDate").val('');
  ...
  $("#deviceSysId_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过20,在园区/当前企业内唯一。', '#deviceSysId_tip');
  }); //设备系统 提示点击 - 设备系统名称
  ...
  $("#sortIdSys_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过10，系统按照字符串升序。', '#sortIdSys_tip');
  }); //设备 提示点击 - 设备标识

  $("#equipId_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过20。在园区/当前企业内唯一。', '#equipId_tip');
  }); //设备 提示点击 - 设备名称
