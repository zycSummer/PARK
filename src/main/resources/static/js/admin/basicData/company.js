 ...
 * @Last Modified time: 2019-12-30 14:26:05
 */
layui.use(['form', 'element', 'layer', 'table', 'jquery', 'laypage'], function () {
  ...
  $("#edit_btn").on("click", function () {
    ...
    request.service({
      ...
      $("#world_longitude").val(tableSelect[0].worldLongitude);
      $("#world_latitude").val(tableSelect[0].worldLatitude);
      ...
      $("#profile").val(tableSelect[0].profile);
  ...
  $("#siteId_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过20。', '#siteId_tip');
  }); //新增企业弹框 提示点击 - 企业名称
  ...
  $("#rtdbProjectId_tip").on("click", function () {
    layer.tips('此企业在实时库中对应的项目标识。\n' + '字母、数字、下划线组合，长度不超过20。', '#rtdbProjectId_tip');
  }); //新增企业弹框 提示点击 - 简介

  $("#profile_tip").on("click", function () {
    layer.tips('长度不超过1000。', '#profile_tip');
  }); //新增企业弹框 提示点击 - 企业图片
  ...
  function add(type, title, editData) {
    layui.layer.open({
      ...
      yes: function yes(index) {
        ...
        if (data.profile.length > 1000) return layer.msg('简介长度不能超过1000！');
        ...
        formData.append("worldLongitude", $("#world_longitude").val());
        formData.append("worldLatitude", $("#world_latitude").val());
