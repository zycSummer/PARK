 ...
 * @Last Modified time: 2019-11-20 13:37:35
 */
...
layui.use(['request','form', 'element', 'layer', 'jquery','slider','tree'], function () {
    ...
    $('.node_add').on('click',function(){
        ...
        $("#treeDetail_memo").val('');
        if(val[0].energyTypeId == 'std_coal'){//标煤
    ...
    （即根据所选参数对应的公式解析出对应的参数列表及公式，当只有一个参数时，公式为空），用户
       ...
       var energyParaId = $(this).attr('data-paraId');
        ...
        $.each(dataSourceArr,function(i,v){
            str += '<tr data-socure="' + v + '"><td>' +
                '<input type="checkbox" name="dataSocurecheckbox['+v+']" value="'+v+'" title="'+v+'" lay-skin="primary"></td></tr>';
        });
    ...
    $("#treeId_tip").on("click", function () {
        layer.tips('字母、数字组合，长度不超过20,在园区/当前企业内唯一。', '#treeId_tip');
    });
    ...
    $("#tree_sortId_tip").on("click", function () {
        layer.tips('字母、数字组合，长度不超过10。', '#tree_sortId_tip');
    });

    //展示结构树节点明细弹框 提示点击 - 节点ID
    $("#treeDetail_nodeId_tip").on("click", function () {
        layer.tips('字母、数字组合，长度不超过20,在当前报表内唯一。', '#treeDetail_nodeId_tip');
    });
    ...
    $("#treeDetail_sortId_tip").on("click", function () {
        layer.tips('字母、数字组合，长度不超过10。', '#treeDetail_sortId_tip');
    });
    /*----------------------------------函数------------------------------------*/
    // 新增修改展示结构树
        ...
        $.each(ds,function(i0,v){
            if(i0 == 0){
                ...
                $.each(dataSourceArr,function(dd,ff){
                    str += '<tr data-socure="' + ff + '"><td>' +
                        '<input type="checkbox" name="dataSocurecheckbox['+ff+']" value="'+ff+'" title="'+ff+'" lay-skin="primary"></td></tr>';
                    ...
                    var setting = {
                        view: {
                            ...
                            fontCss: function (treeId, treeNode) {
                                return treeNode.isUse == true ? {} : {color:"#bbb"};
                            },
                            showIcon: false
                        },
                        data: {
        ...
        if(siteObj && val1.length>0){
            ...
            request.service({
                ...
                if(res.data){
                    ...
                    nodeTableData = [];
                    renderNodeTableData(res.data);console.log(nodeTableData)
                    treeGrid.render({
                        id:'treeDetail_table'
