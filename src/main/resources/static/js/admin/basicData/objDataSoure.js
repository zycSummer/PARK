 ...
 * @Last Modified time: 2019-12-02 10:51:14
 */
layui.use(['form', 'element', 'layer', 'table', 'jquery', 'laypage', 'laydate'], function () {
    ...
    function getEnergyTypes(){  //获取能源种类
        load();
        request.service({
                method: 'get',
     ...
     function renderAddEnergyTypeSelect(data){ //渲染新增修改能源种类选择
     ...
     form.on("select(energyTypeSelect_add)",function(obj){ //能源种类选择事件
     energyTypeIdSelect = obj.value; //能源种类选择值
      renderEnergyPara(obj.value,null);
     })
  

       function renderEnergyPara(energyTypeId,energyParaId){ //根据所选择的能源种类 获取参数列表
           load();
           var formData ={};
            formData.key ='';
     ...
     function renderParaSelect(data,energyParaId){  //参数选择渲染事件
    ...
    function renderDataSourceTable(data) { //渲染数据源表格
        table.render({
    ...
    $(".parameter_btn_type").on('click', function () {  //添加至所选之前
        if ($(".parameter_btn_status").length > 0) { //当存在所选参数时 判断 仪表参数列表是否存在
            ...
            if(energyTypeIdSelect === 'std_coal'){  //当为标煤时
    ...
    $("#del_btn_data").on('click', function () {
        ...
        layer.open({
            ...
            content: '<div id="park_select_tree"> <form class="layui-form" style="margin:30px;">' +
                '   <div class="layui-form-item "><label>能源种类：</label>' +
                '<div class="layui-inline">' + checkStatusData[0].energyTypeId + '</div></div>' +
                '   <div class="layui-form-item "><label>参数：</label>' +
                '<div class="layui-inline" >' + checkStatusData[0].energyParaId + '</div></div>' +
                '   <div class="layui-form-item " style="color:red;">确定要删除此数据源吗？</div>' +
