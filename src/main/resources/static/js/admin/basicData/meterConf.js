 ...
 * @Last Modified time: 2019-12-19 11:03:07
 */
layui.use(['form', 'element', 'layer', 'table', 'jquery', 'laypage'], function () {
  ...
  $('.export').on('click', function () {
    ...
    var url = '/meter/exportExcel?objType=' + siteObj.type + '&objId=' + siteObj.id + '&energyTypeId=' + arr + '&meterId=' + $("#search_meterId").val() + '&meterName=' + $("#search_meterName").val();
    window.location.href = encodeURI(url);
  }); //新增弹框 提示点击 - 仪表标识

  $("#meterId_tip").on("click", function () {
    layer.tips('字母、数字、下划线组合，长度不超过20。\n' + '和实时库中的仪表标识对应。', '#meterId_tip');
  }); //新增弹框 提示点击 - 仪表名称
  ...
  function addModel(title, editId) {
    //模态框调用事件
    layer.open({
      type: 1,
      title: title,
